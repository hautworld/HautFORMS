...
// Lógica para cadastrar/atualizar usuário
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['acao']) && $_POST['acao'] === 'cadastrar_usuario') {
        ...
        $stmt->bind_param("ssssiii", $nome, $sobrenome, $email, $senha_hash, $cargo_id, $departamento_id, $nivel_acesso_id);
        ...
        $stmt->close();
    } else if (isset($_POST['acao']) && $_POST['acao'] === 'editar_usuario') {
        ...
        // Verifica se a senha foi alterada e cria o hash
        if (!empty($senha)) {
            $senha_hash = password_hash($senha, PASSWORD_DEFAULT);
            $sql = "UPDATE usuarios SET nome=?, sobrenome=?, email=?, senha=?, cargo_id=?, departamento_id=?, nivel_acesso_id=? WHERE id=?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssiiii", $nome, $sobrenome, $email, $senha_hash, $cargo_id, $departamento_id, $nivel_acesso_id, $usuario_id);
        } else {
            // Se a senha não for alterada, mantém a antiga
            $sql = "UPDATE usuarios SET nome=?, sobrenome=?, email=?, cargo_id=?, departamento_id=?, nivel_acesso_id=? WHERE id=?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssiiii", $nome, $sobrenome, $email, $cargo_id, $departamento_id, $nivel_acesso_id, $usuario_id);
        }
